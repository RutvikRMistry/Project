 ...
 <div class="page">
  ...
  <div class="page-main">
   <div class="page-header">
      <h1 class="page-title">Customers  <small class="text-muted">Manage your Customers</small></h1>
      <div class="page-header-actions">
         <a href="{{route('backend.admin.pos.contact_book.customer.add')}}">
   ...
   <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
			...
			@foreach($contact as $con)
				<tr>
				   ...
				   <td>{{$con->name}}</td>
				   <td>{{ $con->customer_group_id == 1 ? 'yes' : 'no' }} </td>
				   <td>{{$con->city}},{{$con->state}},{{$con->country}}</td>
				   <td>{{$con->mobile}}</td>
				   <td>work On It Later</td>
				   <td class="actions">
				   ...
				   <a href="#" class="btn btn-sm btn-icon btn-pure btn-default on-editing cancel-row"
					  data-toggle="tooltip" data-original-title="remove" hidden><i class="fa fa-eye" aria-hidden="true"></i></a>
				   <a href="{{route('backend.admin.pos.contact_book.customer.edit',$con->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
					  data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
				  <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
								data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$con->id}},'{{route('backend.admin.pos.contact_book.customer.delete',$con->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
				   <a href="{{route('backend.admin.pos.contact_book.payment',$con->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
					  data-toggle="tooltip" data-original-title="Pay Due To Amount"><i class="fa fa-money" aria-hidden="true"></i></a>
				   <a href="{{route('backend.admin.pos.contact_book.customer.view',$con->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
