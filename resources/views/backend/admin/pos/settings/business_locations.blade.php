...
@section('title',"Buiness Locations")
@section('content')
 ...
 <div class="page">
   ...
   <div class="page-header">
      <h1 class="page-title">Business Locations</h1>
      <div class="page-header-actions">
         <a href="{{route('backend.admin.pos.setting.business_location.add')}}">
               ...
               <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
				...
				@foreach($location as $loc)
                     <tr>
                        ...
                        <td>{{$loc->state}}</td>
                        <td>{{$loc->country}}</td>
                        <td>{{$loc->invoice_scheme_id}}</td>
                        <td>{{$loc->invoice_layout_id}}</td>
                        <td class="actions">
                           ...
                           <a href="{{route('backend.admin.pos.setting.business_location.edit',$loc->id)}}">
                           <button type="button" class="btn btn-primary"> <i class="icon wb-edit">  </i> </button></a>
                           <a href="{{route('backend.admin.pos.setting.business_location.setting',$loc->id)}}">
                           <button type="button" class="btn btn-primary"><i class="icon wb-settings">  </i></button></a>
						   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
								data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$loc->id}},'{{route('backend.admin.pos.setting.business_location.delete',$loc->id)}}');">
								<button type="button" class="btn btn-primary"><i class="icon wb-trash " aria-hidden="true"></i></button></a>
