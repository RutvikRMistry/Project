  ...
  <div class="page-main">
   ...
   <div style="width: 100%; padding: 30px;">
      <div class="panel-body" style="background-color: #fff; ">
         <div>
            <form action="{{route('backend.admin.pos.expenses.store')}}" method="post">
				{{csrf_field()}}
               <div class="row">
                  <div class="col-lg-4">
                     ...
                     <select class="form-control" data-plugin="select2"  id="location_id" name="location_id" required >
                        <option value="">--Select--</option>
                           <option value="{{$business_locations->id}}" {{old('location_id') == $business_locations->id?'selected' :''}}>{{$business_locations->name}}</option>
                     </select>
					 <span class="text-danger">{{$errors->first('location_id')}}</span>
                  </div>
                  <div class="col-lg-4">
                     <h5>Reference No:<span class="text-danger">*</span></h5>
                     <input class="form-control" name="ref_no" type="text" id="ref_no" value="{{old('ref_no')}}" required />
                  ...
                  <div class="col-lg-4">
                     ...
                     <select class="form-control" data-plugin="select2" id="expense_category_id" name="expense_category_id" required >
						...
						@foreach($expense_categories as $expc)
                           <option value="{{$expc->id}}" {{old('expense_category_id') == $expc->id ? 'selected' : ''}}>{{$expc->name}}</option>
                        @endforeach
                     </select>
               ...
               <div class="row">
                  <div class="col-lg-4">
                     ...
                     <div class="input-group">
                        ...
                        <input type="text" class="form-control" data-plugin="datepicker"  readonly=""  id="expense_transaction_date" name="transaction_date" type="text" value="{{date('Y-m-d')}}" required >
                  ...
                  <div class="col-lg-4">
                     ...
                     <input class="form-control input_number" placeholder="Total amount" value="{{old('final_total')}}" name="final_total" type="text" id="final_total" required >
               ...
               <div class="row">
                  <div class="col-lg-4">
                     ...
                     <select class="form-control" data-plugin="select2" id="expense_for" name="expense_for" tabindex="-1" aria-hidden="true" required >
                        ...
                        <option value="{{$user->id}}" {{old('expense_for') == $user->id?'selected' :''}}>{{$user->surname.$user->first_name.$user->last_name}}</option>
                        @endforeach
                     </select>
