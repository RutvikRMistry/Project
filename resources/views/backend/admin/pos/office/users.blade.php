...
@section('content')
 ...
 <div class="page-main">
    <div class="page-header">
        <h1 class="page-title">User  <small class="text-muted">Manage Users</small></h1>
        <div class="page-header-actions">
            <a href="{{route('backend.admin.pos.office.user.add')}}">
	...
	@foreach($users as $user)
		...
		@endphp
			...
			<td>{{$name}}</td>
			<td>{{$user->username}}</td>
			<td>Work On It Later After the roles Section</td>
			<td>{{$user->email}}</td>
			 <td class="actions">
                           ...
                           <a href="{{route('backend.admin.pos.office.user.edit',$user->id)}}">
                           <i class="btn btn-sm btn-icon btn-pure btn-default icon wb-edit">  </i></a>
						   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
								data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$user->id}},'{{route('backend.admin.pos.office.user.delete',$user->id)}}');">
