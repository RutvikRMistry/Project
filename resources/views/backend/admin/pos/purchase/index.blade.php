...
@section('content')
 ...
 <div class="page-main">
   <div class="page-header">
      <h1 class="page-title">All Purchases</h1>
      <div class="page-header-actions">
         <a href="{{route('backend.admin.pos.purchase.add')}}">
            ...
            <div>
               <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
				  ...
				  @foreach($purchases as $pur)
                     <tr>
                        <td>{{$pur->transaction_date}}</td>
                        <td>{{$pur->ref_no}}</td>
                        <td>{{$pur->location_name}}</td>
                        <td>{{$pur->name}}</td>
                        <td>
                           <div class="badge {{$pur->status == 'received'? 'badge-success' : 'badge-danger' }}">{{$pur->status}}</div>
                        </td>
                        <td>
                           <div class="badge {{$pur->payment_status == 'paid'? 'badge-success' : 'badge-danger' }}">{{$pur->payment_status}}</div>
                        </td>
                        ...
                        <td class="actions">
                           ...
                           <a href="{{route('backend.admin.pos.purchase.add')}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                           ...
                           <a href="{{route('backend.admin.pos.product.print_label')}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                              data-toggle="tooltip" data-original-title="Label "><i class="fa fa-barcode" aria-hidden="true"></i></a>
                           <a href="{{route('backend.admin.pos.purchase.view')}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                              data-toggle="tooltip" data-original-title="View"><i class="fa fa-eye" aria-hidden="true"></i></a>
                           <a href="{{route('backend.admin.pos.purchase.payment')}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                              data-toggle="tooltip" data-original-title=" View Payments"><i class="fa fa-money" aria-hidden="true"></i></a>
					 ...
					 $sum =$pur->sum('final_total');
					@endforeach
                     <tr>
                        <td colspan="4" class="bg-secondary text-white">
                           <center>Total</center>
                        </td>
                        <td class="bg-secondary text-white">  Received - 1</td>
                        <td class="bg-secondary text-white">Paid - 1</td>
                        <td class="bg-secondary text-white">remains $  </td>
