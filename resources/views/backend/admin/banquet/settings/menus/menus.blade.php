	...
	<div class="page-main">
	   <div class="page-header">
		  ...
		  <div class="page-header-actions">
			 <a href="{{route('backend.admin.banquet.sttings.menu.add')}}">
				   ...
				   <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
					  ...
					  <tbody>
						@foreach($main_menu as $menu)
						 <tr>
							<td>{{$menu->name}}</td>
							<td>{{$menu->min_person}}</td>
							<td>{{$menu->max_person}}</td>
							<td>{{$menu->tables}}</td>
							<td class="actions">
							   ...
							   <a href="{{route('backend.admin.banquet.sttings.menu.edit',$menu->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
								  data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
							   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
									data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$menu->id}},'{{route('backend.admin.banquet.sttings.menu.delete',$menu->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
							</td>
						 </tr>
						@endforeach
					  </tbody>
