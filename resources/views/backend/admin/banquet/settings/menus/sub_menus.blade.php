    ...
    @include('backend.partials.leftbar.shortcuts')
	  <div class="page-main">
	   <div class="page-header">
		  <h1 class="page-title"> Sub Menu Types</h1>
		  <div class="page-header-actions">
			 <a href="{{route('backend.admin.banquet.sttings.menu.sub_menu.add')}}">
					  ...
					  <tbody>
						@foreach($sub as $sub)
						 <tr>
							...
							<td>{{$sub->type}}</td>
							<td>{{$sub->name}}</td>
							<td>{{$sub->max_person}}</td>
							<td>{{$sub->min_person}}</td>
							<td>{{$sub->times}}</td>
							<td class="actions">
							   ...
							   <a href="{{route('backend.admin.banquet.sttings.menu.sub_menu.edit',$sub->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
								  data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
							   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
									data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$sub->id}},'{{route('backend.admin.banquet.sttings.menu.sub_menu.delete',$sub->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
							</td>
						 </tr>
						@endforeach
					  </tbody>
