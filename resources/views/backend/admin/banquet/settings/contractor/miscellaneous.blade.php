	...
	<div class="page-main">
	   <div class="page-header">
		  ...
		  <div class="page-header-actions">
			 <a href="{{route('backend.admin.banquet.sttings.contractor.miscellaneous.add')}}">
			 ...
			 <header class="panel-heading">
				...
				<div>
				   <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
					  ...
					  <tbody>
						@foreach($miscellaneous as $miscell)
						 <tr>
							<td>{{$miscell->name}}</td>
							<td>{{$miscell->address}}</td>
							<td>{{$miscell->phone}}</td>
							<td>{{$miscell->email}}</td>
							<td class="actions">
							   ...
							   <a href="{{route('backend.admin.banquet.sttings.contractor.miscellaneous.edit',$miscell->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
								  data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
							   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
									data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$miscell->id}},'{{route('backend.admin.banquet.sttings.contractor.miscellaneous.delete',$miscell->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
							</td>
						 </tr>
						@endforeach
					  </tbody>
