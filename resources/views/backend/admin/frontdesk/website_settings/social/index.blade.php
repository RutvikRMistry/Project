...
@section('content')
 ...
 <div class="page">
   ...
   <div class="page-header">
      <h1 class="page-title">Social Item</h1>
   </div>
         ...
         <header class="panel-heading">
            ...
            <div style="width: 50%; float: right;">
               <div class="btn-group" aria-label="Button group with nested dropdown" role="group" style="float: right; padding-top: 10px;">
                  <a href="{{route('backend.admin.frontdesk.website_setting.social.create')}}">
                     <button type="button" class="btn btn-primary" style="margin: auto;"><i class="icon wb-plus mr-10" aria-hidden="true"></i>Add New</button>
				 </a>
			   </div>
            </div>
            <div>
            ...
            <tr class="gradeA">
            <td>{{$social->name}}</td>
            <td><span class="{{$social->icon}} " >&nbsp;&nbsp;
				<span class="bg-white">{{$social->icon}}</span>
				</span>
			</td>
            <td>{{$social->link}}</td>
            <td><span  style="color:{{$social->color}}">{{$social->color}}</span></td>
            <td>
			<span class="badge  badge-pill  badge-{{ $social->status ===0 ? 'warning' : 'success' }}">{{ $social->status === 0 ? 'Deactive' : 'Active' }}</span> 
			</td>
            <td class="actions">
            ...
            <a href="{{route('backend.admin.frontdesk.website_setting.social.update',$social->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
               data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
			   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
								data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$social->id}},'{{route('backend.admin.frontdesk.website_setting.social.delete',$social->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
            </td>
            </tr>
			@endforeach
            </tbody>
