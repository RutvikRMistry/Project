...
@section('content')
 ...
 <div class="page">
   ...
   <div class="page-header">
      <h1 class="page-title">Home ( Counter section )</h1>
   </div>
         ...
         <header class="panel-heading">
            ...
            <div style="width: 50%; float: right;">
               <div class="btn-group" aria-label="Button group with nested dropdown" role="group" style="float: right; padding-top: 10px;">
                  <a href="{{route('backend.admin.frontdesk.website_setting.home.counter.add')}}">
            ...
            <div>
               <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
                  ...
                  <tbody>
					...
					@foreach($web_counter as $counter)
						 <tr class="gradeA">
							<td>{{$cnt++}}</td>
							<td>{{$counter->name}}</td>
							...
							<td class="actions">
							   ...
							   <a href="{{route('backend.admin.frontdesk.website_setting.home.counter.edit',$counter->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
								  data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
							   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
									data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$counter->id}},'{{route('backend.admin.frontdesk.website_setting.home.counter.delete',$counter->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
							</td>
						 </tr>
					@endforeach	
