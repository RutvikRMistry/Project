         ...
         <header class="panel-heading">
            ...
            <div style="width: 50%; float: right;">
               <div class="btn-group" aria-label="Button group with nested dropdown" role="group" style="float: right; padding-top: 10px;">
                  <a href="{{route('backend.admin.frontdesk.website_setting.blog.category.create')}}">
                  <button type="button" class="btn btn-primary" style="margin: auto;"><i class="icon wb-plus mr-10" aria-hidden="true"></i>Add Category</button></a>
               </div>
            </div>
            <div>
               <table class="table table-hover dataTable table-striped w-full dtr-inline" data-plugin="dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="width: 1783px;">
                  ...
                  <tbody>
					...
					<tr>
						...
						<td>{{$cnt++}}</td>
						<td>{{$category->name}}</td>
						<td><span class="badge  badge-pill  badge-{{ $category->status ==0 ? 'warning' : 'success' }}">{{ $category->status == 0 ? 'Deactive' : 'Active' }}</span> </td>
						<td class="actions">
                              ...
                              <a href="{{route('backend.admin.frontdesk.website_setting.blog.category.edit',$category->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
                                 data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
								<a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
								data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$category->id}},'{{route('backend.admin.frontdesk.website_setting.blog.category.delete',$category->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
