			 ...
			 <header class="panel-heading">
				...
				<div style="width: 50%; float: right;">
				   <div class="btn-group" aria-label="Button group with nested dropdown" role="group" style="float: right; padding-top: 10px;">
					  <a href="{{route('backend.admin.frontdesk.hotel_configure.floor.create')}}">
					  <button type="button" class="btn btn-primary" style="margin: auto;"><i class="icon wb-plus mr-10" aria-hidden="true"></i>Add Floor </button></a>
				   </div>
				</div>
				<div>
					  ...
					  @foreach($floors as $key=>$floor)
                        ...
                        <td><span class="badge {{$floor->status?'badge-success':'badge-danger'}}">{{$floor->status?'Active':'Inactive'}}</span></td>
							<td class="actions">
							   ...
							   <a href="{{route('backend.admin.frontdesk.hotel_configure.floor.edit',$floor->id)}}" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row"
								  data-toggle="tooltip" data-original-title="Edit"><i class="icon wb-edit" aria-hidden="true"></i></a>
							   <a href="javascript:void(0);" class="btn btn-sm btn-icon btn-pure btn-default on-default remove-row"
								data-toggle="tooltip" data-original-title="Remove" onclick="deleteBlog({{$floor->id}},'{{route('backend.admin.frontdesk.hotel_configure.floor.delete',$floor->id)}}');"><i class="icon wb-trash" aria-hidden="true"></i></a>
						   </td>
						 </tr>
						@endforeach
					  </tbody>
